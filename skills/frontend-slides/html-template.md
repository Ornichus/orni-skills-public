...
**Use when:** Phase 1 Q5 returns B (flowchart), C (timeline/Gantt), D (architecture), E (data/KPI pie), or F (mix).

**Mermaid v11 bug:** By default the library applies `useMaxWidth: true` and inlines `max-width` on the SVG — which shrinks every diagram to ~200px regardless of container size. Two-part fix:

1. **CSS rules** — already in `viewport-base.css` section 9 (`.mermaid svg { width: 100% !important; ... }`). Include them by pasting the full viewport-base.css contents into the deck's `<style>` block.

2. **JS init** — override the defaults per chart type. Add right before `new SlidePresentation();`:

```html
<!-- Mermaid v11 (load before slide controller) -->
<script src="https://cdn.jsdelivr.net/npm/mermaid@11/dist/mermaid.min.js"></script>
<script>
  // useMaxWidth: false on every chart type forces SVGs to fill their wrapper.
  // Without this, diagrams render at ~200px and look broken on a 1080p slide.
  mermaid.initialize({
    startOnLoad: true,
    theme: 'dark', // or 'default' / 'forest' / 'neutral' — match the chosen style preset
...
**Slide pattern** — one diagram per slide, wrapped in a `.mermaid` div. The content inside the div is raw Mermaid DSL (not HTML):

```html
<section class="slide diagram-slide">
  <div class="slide-content">
    <h2 class="reveal">Architecture</h2>
    <div class="mermaid reveal">
      graph LR
...
**Theme mapping** — match the preset:

| Preset family               | Mermaid theme | Notes                                  |
| --------------------------- | ------------- | -------------------------------------- |
| Neon Cyber, Terminal Green  | `dark`        | + custom `themeVariables` for accents  |
| Bold Signal, Electric Studio| `dark`        | Override `primaryColor` to brand accent|
| Notebook Tabs, Paper & Ink  | `default`     | Light background fits cream presets    |
| Vintage Editorial           | `neutral`     | Muted palette matches cream + serif    |

For tighter brand control, pass `themeVariables` to `mermaid.initialize(...)`:
...
**Density** — see SKILL.md "Content Density Limits" table: a diagram slide is 1 heading + 1 diagram + max ~60 words of caption. If you need more explanation, split into a text-only follow-up slide.
