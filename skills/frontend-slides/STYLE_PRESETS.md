...
**Le style maison de référence, validé en production.** Branché via `--style nimbe` (preset complet : tokens, verre, composants). Les 12 presets ci-dessous restent disponibles pour les autres contextes.

**Vibe :** premium corporate, atmosphérique, interactif. Light par défaut, dual dark/light.

**Layout :** halos de lumière colorée animés en fond → surface en **verre dépoli** (`.lg`) qui les réfracte. Grille fine masquée en radial + grain léger.

**Typography:**
- Display: `Outfit` (700/800)
- Body: `DM Sans` (400/500)
- Mono: `JetBrains Mono` (eyebrows, KPI labels, pieds de page)

**Colors (light) :**
```css
:root {
  --nimbe-blue:#0027a3; --nimbe-red:#a1140e; --nimbe-cyan:#16b6c9;   /* accents décoratifs */
  --ink:#0d1430; --ink-soft:#39406a; --muted:#6f779c;          /* encre neutre */
  /* Tokens texte dual-theme — À UTILISER en color: (foncés light / clairs dark) */
  --c-blue:#0027a3; --c-red:#a1140e; --c-cyan:#0f8896; --c-green:#15703f;
}
```

**Signature Elements:**
- Surface verre `.lg` (reflet spéculaire + sheen interactif au survol)
- **Tokens texte `--c-*`** dual-theme — jamais `var(--nimbe-*)` brut en couleur de texte (illisible en dark)
- **Cyan `#16b6c9` réservé aux éléments « Équipe »** (lignes de table `.row-team`, tags, badges)
- **Configurateur à règle métier ancrée** : état `.invalid` (cadre rouge + bandeau + correctif 1-clic) quand une invariante est violée
- KPI à dégradé, tables `.ptable`, segment toggle, reveals au scroll (`.rv`)

**Socle :** `styles/nimbe/golden/LIQUID-GLASS-STANDARD-v2.html` (standard du verre) + `styles/nimbe/preset.css`. Démo composants : `styles/nimbe/components/SHOWCASE.html`.

> `.lg` = nom technique de la surface en verre. **Nimbe** = nom du style.
...
## [DIFFERE] Engineering Brief preset

A "doc archi sérieuse" preset for engineering briefs, runbooks, post-mortems, internal architecture decks with heavy Mermaid usage. Two seeds proposed but not built — needs user validation before scaffolding a full preset (preset.css + style doc).

- **Seed A — Terminal Green mono-only** — same palette as preset #10 but Mermaid-first: large diagram area (70vh), JetBrains Mono for everything including diagram labels, scan lines kept subtle, no cursor animation. Best for SRE / infra decks.
- **Seed B — Neon Cyber dimmed** — preset #9 with the neon turned down 40% (no glow, no particles), keeps cyan as Mermaid `lineColor` but uses a slate-gray body for prose. Best for architecture proposals where readability beats flair.

**Status :** not implemented. Falling back to Terminal Green (#10) or Neon Cyber (#9) is fine in the meantime — just dial down animation density.
